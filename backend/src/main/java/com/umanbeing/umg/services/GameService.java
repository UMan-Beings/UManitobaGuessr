...
/**
 * Service class for managing game-related operations. Provides methods for creating, retrieving,
 * and updating game data.
 */
@Service
public class GameService {
   ...
   * @throws ResponseStatusException if the input parameters are invalid，handled by
  ...
  public Game createNewGame(int totalRounds, int maxTimerSeconds, Long userId) {
    if (totalRounds <= 0) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Total rounds must be greater than 0");
    }
    if (totalRounds > MAX_ROUNDS) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Total rounds must not exceed " + MAX_ROUNDS);
    }
    if (maxTimerSeconds < 0) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Max timer seconds must be non-negative");
    }
    if (maxTimerSeconds > MAX_TIME_LIMIT_SECONDS) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Max timer seconds must not exceed " + MAX_TIME_LIMIT_SECONDS);
    }
    Game game = new Game();
  ...
  /**
   * Submits a guess for the current round of the game. Validates the input parameters and throws
   ...
   */
  @Transactional
  public Game submitGuess(
      Long gameId, BigDecimal guessedX, BigDecimal guessedY, Long guessTimeSeconds) {
    Game game = getGameById(gameId);
    if (GameState.GUESS != game.getGameState()) {
      throw new ResponseStatusException(
          HttpStatus.CONFLICT, "Game is not in a state to accept guesses");
    }
    if (guessedX == null || guessedY == null) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Guessed coordinates must not be null");
    }
   ...
   * @param gameId the ID of the game
   * @return the updated Game object
   * @throws ResponseStatusException if the game does not have a timer set or is not in a state to
   *     accept guesses
   */
  @Transactional
  ...
  /**
   * Proceeds to the next round of the game. The game must be in the REVEAL state to proceed.
   *
   * @param gameId the ID of the game
   * @return the updated Game object
   * @throws ResponseStatusException if the game is not in a state to proceed to the next round,
  ...
  public Game nextRound(Long gameId) {
    ...
    if (GameState.REVEAL != game.getGameState()) {
      throw new ResponseStatusException(
          HttpStatus.CONFLICT, "Game is not in a state to proceed to the next round");
    }
   ...
   * @throws ResponseStatusException if the user ID is null or user game stats are not found,
   *     handled by GlobalExceptionHandler.
   */
