...
public class GuessService {
   ...
   * @throws ResponseStatusException if the guessed coordinates are null, handled by
   *     GlobalExceptionHandler.
   */
  public Guess createGuess(
      Round round, BigDecimal guessedX, BigDecimal guessedY, Long guessTimeSeconds) {
   ...
   */
  private void setGuessAndScore(
      Guess guess, Round round, BigDecimal guessedX, BigDecimal guessedY) {
    Location location = round.getLocation();
    Integer distance =
        calculateDistance(guessedX, guessedY, location.getCorX(), location.getCorY());
   ...
   * @throws ResponseStatusException if data integrity violation occurs, handled by
   *     GlobalExceptionHandler.
   */
  ...
  private Integer calculateDistance(BigDecimal x1, BigDecimal y1, BigDecimal x2, BigDecimal y2) {
    return (int)
        Math.hypot(x1.doubleValue() - x2.doubleValue(), y1.doubleValue() - y2.doubleValue());
  }
  ...
  private int calculateScore(int distance) {
    ...
    if (distance <= FULL_SCORE_DISTANCE) {
      ...
      calculatedScore =
          Math.max(0, (int) Math.round(MAX_SCORE * (1 - scaledDistance / scoringRange)));
    }
