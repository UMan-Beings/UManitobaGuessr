...
/**
 * Service class for user authentication and authorization. Provides methods for user registration,
 * login, and token generation.
 *
 * <p>It is very similar to the UserService class, but focuses on authentication and token
 * generation.
 */
@Component
@AllArgsConstructor
public class AuthService implements UserDetailsService {
   ...
   * load here. In this case, we are loading the user's email and password.
   *
   ...
   */
  public SignUpResponse registerUser(CreateAccountRequest createAccountRequest)
    ...
    return new SignUpResponse("User registered successfully", savedUser.getUsername());
  }
  ...
  public LoginResponse loginUser(LoginRequest loginRequest) throws IllegalArgumentException {
    try {
      ...
      Authentication auth =
          authenticationManager.authenticate(AuthMapper.fromDtoLogin(loginRequest));
      // Generate JWT token
