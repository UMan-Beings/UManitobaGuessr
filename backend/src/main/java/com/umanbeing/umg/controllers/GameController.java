 ...
 *
 * <p>This controller provides endpoints for game management, including creating games, making
 * guesses, and retrieving game state. Each handler validates and processes the request body
 * according to the corresponding DTO.
 *
 * <p>GameService is responsible for handling game logic, while UserService manages user-related
 * operations.
 */
@RestController
...
public class GameController {
  ...
  public ResponseEntity<Map<String, Object>> createNewGame(
      @RequestBody CreateGameRequest request, Authentication authentication) {
    User user = getAuthenticatedUserOrNull(authentication);
    Long userId = user != null ? user.getUserId() : null;
    Game game =
        gameService.createNewGame(request.getTotalRounds(), request.getMaxTimerSeconds(), userId);
  ...
  public ResponseEntity<Map<String, Object>> makeGuess(
      ...
      @RequestBody MakeGuessRequest request,
      Authentication authentication) {
    checkGameOwnership(gameService.getGameById(gameId), authentication);
    Game game =
        gameService.submitGuess(
            gameId, request.getCorX(), request.getCorY(), request.getGuessTimeSeconds());
    Round currentRound = game.getCurrentRound();
    Guess guess = currentRound.getGuess();
    Map<String, Object> response = new HashMap<>();
    response.put(JSON_PHASE, game.getGameState());
  ...
  /**
   * Checks if the given game belongs to the authenticated user. It throws a ResponseStatusException
   * if the game does not belong to the authenticated user.
   *
